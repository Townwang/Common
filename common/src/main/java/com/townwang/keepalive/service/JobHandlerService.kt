...
class JobHandlerService : JobService() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        @Suppress("NAME_SHADOWING") var startId = startId
        startService(this)
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            mJobScheduler = getSystemService(Context.JOB_SCHEDULER_SERVICE) as JobScheduler
            @Suppress("UNUSED_CHANGED_VALUE") val builder = JobInfo.Builder(startId++,
    ...
    override fun onStartJob(jobParameters: JobParameters): Boolean {
        if (!isServiceRunning(applicationContext, "com.townwang.keepalive.service.LocalService") || !isServiceRunning(applicationContext, "$packageName:remote")) {
            startService(this)
        }
        return false
    }
    ...
    private fun isServiceRunning(ctx: Context, className: String): Boolean {
        ...
        @Suppress("DEPRECATION") val servicesList = activityManager
